...
document.getElementById('transactionForm').addEventListener('submit', async function (event) {
    ...
    if (isEditing) {
        ...
        await updateTotalTransactionAmount(); // async/await
    } else {
...
function addTransaction(payee, amount, reason) {
    ...
    // task4: Push newTransaction into transactions array
    transactions.push(newTransaction);
}

function updateTransaction(id, payee, amount, reason) {
    // task5: Find the transaction with matching ID
...
async function deleteTransaction(id) {
    ...
    if (index !== -1) {
        // task6: Remove transaction from array using splice
...
// task7 + enhanced: Make this function async
